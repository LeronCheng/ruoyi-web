...
import { getInitChat, mlog, subModel, getSystemMessage, localSaveAny, canVisionModel, isTTS, subTTS, file2blob, whisperUpload, getHistoryMessage, checkDisableGpt4, chatSetting, isCanBase64Model } from '@/api'
...
watch(
  ...
  async n => {
    if (n == 'gpt.submit' || n == 'gpt.whisper') {
      ...
      let message = [{ role: 'system', content: getSystemMessage(+st.value.uuid) }, ...historyMesg]
      textRz.value = []
      ...
      subTTS({ model: 'tts-1', input: actData.text, voice: nGptStore.tts_voice })
        .then(d => {
...
const submit = (model: string, message: any[], opt?: any) => {
  ...
  if (model == 'whisper-1') {
    ...
    subTTS({ model, input: text, voice: nGptStore.tts_voice })
      .then(d => {
        mlog('subTTS', d)
        //d.player.play();
        //textRz.value.push('ok');
