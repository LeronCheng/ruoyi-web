...
onMounted(() => {
  fetchData(), fetchDataGetKnowledge()
})
...
const voiceList = computed(() => {
  let rz = []
  for (let o of 'alloy,echo,fable,onyx,nova,shimmer'.split(/[ ,]+/gi)) rz.push({ label: o, value: o })
  return rz
})
const modellist = computed(() => {
  ...
  let uniqueArray: { label: string; value: string }[] = Array.from(new Map(rz.map(item => [JSON.stringify(item), item])).values())
  return uniqueArray
})
const ms = useMessage()
...
watch(
  ...
  n => {
    ...
    if (n.indexOf('vision') > -1) {
      max = 4096
    } else if (n.indexOf('gpt-4') > -1 || n.indexOf('16k') > -1) {
