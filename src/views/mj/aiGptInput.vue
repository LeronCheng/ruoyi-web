...
import { NInput, NButton, useMessage, NImage, NTooltip, NAutoComplete, NTag, NPopover, NModal, NDropdown } from 'naive-ui'
import { SvgIcon, PromptStore } from '@/components/common'
import { canVisionModel, GptUploader, mlog, upImg, getFileFromClipboard, isFileMp3, countTokens, checkDisableGpt4, Recognition, chatSetting } from '@/api'
...
<template>
  ...
  <div v-else>
    <div class="flex items-base justify-start pb-1 flex-wrap-reverse" v-if="st.fileBase64.length > 0" style="margin: 0 40px">
    ...
    <div class="myinputs" :class="[!isMobile ? 'chat-footer' : '']" @drop="drop" @paste="paste">
      <div class="top-bar" v-if="!isMobile">
        <div class="left" v-if="st">
          <div v-if="homeStore.myData.local != 'draw'" class="chage-model-select" @click="st.isShow = true">
          ...
          <n-tooltip trigger="hover">
            <template #trigger>
              ...
              <IconSvg icon="upload" @click="fsRef.click()" v-else width="22px" height="22px"></IconSvg>
            </template>
            <div v-if="canVisionModel(gptConfigStore.myData.model)" v-html="$t('mj.upPdf')"></div>
            <div v-else v-html="$t('mj.upImg')"></div>
          </n-tooltip>
          <IconSvg @click="handleExport" icon="screenshot" width="22px" height="22px"></IconSvg>
          <!-- <n-tooltip trigger="hover">
						<template #trigger>
        ...
        <IconSvg @click="handleClear" class="right" icon="clear" width="28px" height="22px"></IconSvg>
        <!-- <div @click="show = true">
            {{ $t('store.siderButton') }}
        </div> -->
      </div>
      ...
      <NAutoComplete v-model:value="mvalue" :options="searchOptions" :render-label="renderOption" :class="[!isMobile ? 'chat-input' : '']">
        <template #default="{ handleInput, handleBlur, handleFocus }">
          <NInput ref="inputRef" v-model:value="mvalue" type="textarea" :placeholder="placeholder" rows="3" :autosize="{ minRows: 3, maxRows: 3 }" @input="handleInput" @focus="handleFocus" @blur="handleBlur" @keypress="handleEnter">
