...
async function getModelList() {
  try {
    ...
    res.data.forEach(item => {
      if (item.category === 'vector') {
        getVectorModel.push(item)
      }
    })
  } catch (error) {
...
<template>
  ...
  <n-drawer class="knowledge-drawer" v-model:show="active" :width="800" :placement="placement" display-directive="show" :mask-closable="false">
    <n-drawer-content :title="$t('knowledge.createKnowledgeBase')" class="drawer-content" closable>
      <n-space vertical>
        <n-form ref="formRef" :label-width="100" :model="formValue" class="knowledge-form" label-placement="left">
          <n-grid :cols="24" :x-gap="24" :y-gap="16">
            ...
            <n-gi :span="12">
              <n-form-item label="分隔符">
                <n-input v-model:value="formValue.knowledgeSeparator" placeholder="请输入知识分隔符" clearable />
              </n-form-item>
            </n-gi>

            <n-gi :span="12">
              <n-form-item label="检索条数" required>
                <n-input-number v-model:value="formValue.retrieveLimit" placeholder="请输入检索条数" :min="1" :max="10" class="full-width" />
              </n-form-item>
            </n-gi>

            <n-gi :span="12">
              <n-form-item label="文本块大小" required>
                <n-input-number v-model:value="formValue.textBlockSize" placeholder="请输入文本块大小" :min="100" :max="2000" class="full-width" />
              </n-form-item>
            </n-gi>

            <n-gi :span="12">
              <n-form-item label="重叠字符">
                <n-input-number v-model:value="formValue.overlapChar" placeholder="请输入重叠字符数" :min="0" :max="200" class="full-width" />
              </n-form-item>
            </n-gi>

            <n-gi :span="12">
              <n-form-item label="向量库" required>
                <n-select :options="getVector" v-model:value="formValue.vector" placeholder="请选择向量库" clearable></n-select>
              </n-form-item>
            </n-gi>
            ...
            <n-gi :span="12">
              <n-form-item label="向量模型" required>
                <n-select :options="getVectorModel" v-model:value="formValue.vectorModel" value-field="modelDescribe" label-field="modelName" placeholder="请选择向量模型" clearable></n-select>
              </n-form-item>
            </n-gi>

            <n-gi :span="24">
              <n-form-item :label="$t('knowledge.knowledgeDescription')">
                <n-input maxlength="1000" type="textarea" v-model:value="formValue.description" :placeholder="$t('knowledge.enterKnowledgeDescription')" :autosize="{ minRows: 3, maxRows: 5 }" show-count />
              </n-form-item>
            </n-gi>

            <n-gi :span="24">
              <n-form-item label="是否公开" label-placement="left">
                <n-switch size="large" checked-value="1" unchecked-value="0" @update:value="handleUpdateValue">
